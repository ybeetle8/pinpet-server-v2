...
async fn main() {
    ...
    if config.solana.enable_event_listener {
        ...
        // 创建涨跌幅存储实例 / Create change storage instance
        let change_storage = Arc::new(change::ChangeStorage::new(db_storage.db.clone()));
    ...
    // 创建涨跌幅存储实例 (用于API查询) / Create change storage instance (for API queries)
    let change_storage_for_api = Arc::new(change::ChangeStorage::new(db_storage.db.clone()));
